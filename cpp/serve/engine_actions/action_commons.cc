...
void ActionStepPostProcess(Array<Request> requests, EngineState estate, Array<Model> models,
  ...
  for (Request request : requests) {
    ...
    std::vector<int64_t> group_delta_tokens_from_retokenize;
    ...
    for (int i = 0; i < n; ++i) {
      ...
      group_delta_tokens_from_retokenize.push_back(delta_request_ret.delta_tokens_from_retokenize);
      group_delta_string_from_retokenize.push_back(delta_request_ret.delta_string_from_retokenize);
      ...
      if (!delta_request_ret.delta_token_ids.empty() ||
          !delta_request_ret.delta_string_from_retokenize.empty()) {
        invoke_callback = true;
      }
    }
    ...
    if (invoke_callback) {
      callback_delta_outputs.push_back(RequestStreamOutput(
          ...
          std::move(group_finish_reason), std::move(group_delta_tokens_from_retokenize),
          std::move(group_delta_string_from_retokenize)));
    }
  }
