  ...
  Array<Request> Step(EngineState estate) final {
    ...
    for (auto rsentry : running_rsentries) {
      ...
      for (; it_past_tokens != mstate->committed_tokens.end(); ++it_past_tokens) {
        past_tokens.push_back(it_past_tokens->GetTokenId());
      }

      auto past_string = tokenizer_->DecodeNoStripSpace(past_tokens);
      auto new_tokens = tokenizer_->EncodeNoPrependSpace(past_string + jump_forward_str);
      ...
      if (rollback_len >
        ...
        // handle output
        std::vector<int32_t> tokens_for_next_callback;
        for (auto i = rsentry->next_callback_token_pos; i < mstate->committed_tokens.size(); ++i) {
          tokens_for_next_callback.push_back(mstate->committed_tokens[i].GetTokenId());
        }
        rsentry->delta_string_from_retokenize +=
            tokenizer_->DecodeNoStripSpace(tokens_for_next_callback) + jump_forward_str;
        ...
        rsentry->delta_tokens_from_retokenize +=
      ...
      if (rollback_len > mstate->num_pending_kv_cache_tokens) {
        models_[0]->PopNFromKVCache(mstate->internal_id,
                                    rollback_len - mstate->num_pending_kv_cache_tokens);
      }

      mstate->num_pending_kv_cache_tokens =
          std::max(0, mstate->num_pending_kv_cache_tokens - rollback_len);
