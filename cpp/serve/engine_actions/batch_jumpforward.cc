...
class BatchJumpForwardActionObj : public EngineActionObj {
  ...
  Array<Request> Step(EngineState estate) final {
    ...
    for (auto rsentry : running_rsentries) {
      ...
      auto [rollback_cnt, new_tokens] =
          GetNewTokensMayRetokenize(mstate, jump_forward_str, MAX_ROLLBACK_TOKENS_);
 ...
 private:
  ...
  std::pair<int, std::vector<int32_t>> GetNewTokensMayRetokenize(RequestModelState mstate,
