...
void RequestModelStateNode::CommitToken(SampleResult sampled_token) {
  ...
  ++num_pending_kv_cache_tokens;

  // std::cout << "Commit token: " << sampled_token.GetTokenId() << " <"
  //           << DebugGetTokenizer()->IdToToken(sampled_token.GetTokenId()) << ">"
  //           << std::endl;
...
void RequestModelStateNode::RollbackTokens(int count) {
  ...
  for (int i = 0; i < count; ++i) {
    // std::cout << "Rollback token: " << committed_tokens.back().GetTokenId() << " <"
    //           << DebugGetTokenizer()->IdToToken(committed_tokens.back().GetTokenId())
    //           << ">" << std::endl;
...
DeltaRequestReturn RequestStateEntryNode::GetDeltaRequestReturn(
  ...
  int delta_tokens_from_retokenize = this->delta_tokens_from_retokenize;
  this->delta_tokens_from_retokenize = 0;
  String delta_string_from_retokenize = this->delta_string_from_retokenize;
  this->delta_string_from_retokenize.clear();
  ...
  if (this->next_callback_token_pos == num_committed_tokens &&
      delta_string_from_retokenize.empty()) {
    return {{}, {}, Optional<String>(), delta_tokens_from_retokenize, delta_string_from_retokenize};
  }
  ...
  if (finish_reason.defined()) {
    return {return_token_ids, logprob_json_strs, finish_reason, delta_tokens_from_retokenize,
  ...
  if (request->generation_cfg->max_tokens >= 0 &&
    ...
    return {return_token_ids, logprob_json_strs, String("length"), delta_tokens_from_retokenize,
            delta_string_from_retokenize};
  }
  ...
  return {return_token_ids, logprob_json_strs, Optional<String>(), delta_tokens_from_retokenize,
          delta_string_from_retokenize};
}
