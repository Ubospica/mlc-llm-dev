...
void GrammarStateMatcherNodeImpl::FindNextTokenBitmask(DLTensor* next_token_bitmask) {
  ...
  // For every stack, we will find the final accepted set or the rejected set (depends on which one
  // is saved in catagorized_tokens). We will inspect all inspect all uncertain tokens to find if
  // it is accepted or rejected, and add it to the result.

  // The rejected token set for all stacks: intersection(rejected_set_i) / union(accepted_set_j)

  // Accepted token bitset. The index is token id.
  tmp_accepted_bitset_.Reset();
  // These indices are the indices in sorted_token_table, instead of the token ids.
  // Initially the rej set being {-1} means the universal set, i.e. all tokens
  ...
  for (auto top : latest_stack_tops) {
    ...
    // rejected indices in uncertain_indices
